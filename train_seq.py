"""
train_seq.py — Contrastive training for MATCHA.

Trains a ContrastiveModel using triplet-style (reference, positive, negative)
contrastive loss with cosine similarity. Uses a StreamingDataset backed by
parquet files so large mixed-source training data need not fit in RAM.
Supports GPT-2, GPT-Neo, Mistral, RoBERTa, and XLM-RoBERTa backbones.
...
from dataset.dataset_streaming import StreamingDataset, collate_fn
...
def train(model, accelerator, train_loader, val_loader, optimizer,
          num_epochs, start_epoch, start_step, best_loss,
          device, output_path, decay_rate, eval_steps, margin):
    """Full training loop with periodic and mid-epoch evaluation.

    Batches are dicts with 'premise'/'correct'/'incorrect' keys
    (from StreamingDataset + collate_fn). Saves 'last_model' at every eval
    checkpoint and 'max_diff' when the (pos_sim - neg_sim) gap improves.
    Also runs an extra evaluation at the midpoint of each epoch.
    """
    total_steps = 0
    max_diff = 0
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------------
    train(
        contrastive_model, accelerator, train_loader, val_loader, optimizer,
